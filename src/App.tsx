...
function App() {
  ...
  const addCustomLight = useCallback(()=>{
    ...
    setSelectedLights(p=>[...p,{id:Date.now().toString(),name:customLightName,lumen:customLightLumen,watt:customLightWatt,
                ...
                {selectedLightColorTemps[light.name] && (
                  <div className="flex flex-col items-center gap-3 pt-4 border-t">
                    <div className="flex items-center">
                      <button onClick={()=>setTempQuantities(p=>{
                                const cur=p[light.name]||1
                                return {...p,[light.name]:Math.max(1,+cur-1)}
                              })}
                              className="w-10 h-10 ring-1 ring-gray-200 rounded-l-lg">−</button>
                      ...
                      <button onClick={()=>setTempQuantities(p=>{
                              ...
                              className="w-10 h-10 ring-1 ring-gray-200 rounded-r-lg">+</button>
                    </div>
                    <button onClick={()=>{
